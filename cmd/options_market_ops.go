...
// optionsMarketStatusCmd retrieves the current real-time trading status
...
// trading. Options traders use this to confirm whether options contracts
// can currently be traded on each exchange.
// Usage: massive options market-status
var optionsMarketStatusCmd = &cobra.Command{
	Use:   "market-status",
	Short: "Get current market status for options exchanges",
	Long:  "Retrieve the real-time open/closed status of US stock exchanges (NYSE, NASDAQ, OTC), currency markets (crypto, forex), and major index groups. Useful for confirming whether options contracts can currently be traded.",
	Args:  cobra.NoArgs,
...
// optionsMarketHolidaysCmd retrieves the list of upcoming market holidays
// and early-close days for NYSE, NASDAQ, and OTC exchanges. This is useful
// for options traders planning around expiration dates that may fall near
// market closures or shortened trading sessions.
// Usage: massive options market-holidays
var optionsMarketHolidaysCmd = &cobra.Command{
	Use:   "market-holidays",
	Short: "Get upcoming market holidays",
	Long:  "Retrieve upcoming market holidays and early-close days for NYSE, NASDAQ, and OTC exchanges, including open/close times for shortened sessions. Helpful for planning around options expiration dates near market closures.",
	Args:  cobra.NoArgs,
	RunE: func(cmd *cobra.Command, args []string) error {
		client, err := newClient()
		if err != nil {
...
// subcommands of the options parent command.
func init() {
	optionsCmd.AddCommand(optionsMarketStatusCmd)
	optionsCmd.AddCommand(optionsMarketHolidaysCmd)
}
